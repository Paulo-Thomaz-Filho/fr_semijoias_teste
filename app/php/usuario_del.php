...
// 3. Desativar usuraio (somente POST)
...
$usuraio_id = $data['usuraio_id'] ?? null;

// 6. Validação
if (!$usuraio_id) {
    echo json_encode(["success" => false, "error" => "ID do usuraio inválido."]);
    exit;
}

// 7. Atualização do campo ativo para 0
try {
    $sql = "UPDATE usuraio 
               SET ativo = 0 
             WHERE id = :usuraio_id";
    ...
    $stmt->bindParam(':usuraio_id', $usuraio_id, PDO::PARAM_INT);

    if ($stmt->execute()) {
        echo json_encode(["success" => true, "message" => "usuraio desativado com sucesso."]);
    } else {
        echo json_encode(["success" => false, "error" => "Não foi possível desativar o usuraio."]);
    }
